...
import { AIContextSchema, type AIContext, type AIContextInput, type AIContextProvider } from "./index.js";

const DENYLIST = ["apiKey", "privateKey", "jwt", "accountId", "secret", "token", "password", "credential"];

const MARKET_CONTEXT_TOOL: Anthropic.Tool = {
...
function buildPrompt(input: AIContextInput): string {
  const safeFeatures = Object.fromEntries(
    Object.entries(input.features).filter(([key]) => !DENYLIST.some((d) => key.toLowerCase().includes(d)))
  );
  const safePortfolio = Object.fromEntries(
    Object.entries(input.portfolioState).filter(([key]) => !DENYLIST.some((d) => key.toLowerCase().includes(d)))
  );
  const safePolicy = Object.fromEntries(
    Object.entries(input.riskPolicy).filter(([key]) => !DENYLIST.some((d) => key.toLowerCase().includes(d)))
  );
...
function conservativeFallback(reason: string): AIContext {
  return {
...
export class ClaudeAIContextProvider implements AIContextProvider {
  private readonly client: Anthropic;
  private readonly model: string;

  constructor(apiKey: string, model = "claude-sonnet-4-6", client?: Anthropic) {
